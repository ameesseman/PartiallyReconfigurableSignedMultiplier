...
component mysmult_static_v1_0
        port (
            -- Users to add ports here
            switch : in std_logic;              --actually a button in real system
    ...
    -- User added
    --
    signal switch : std_logic := '0';           --actually a button in real system
   ...
   signal AXI_WDATA_IN: chunkB;
    ...
    begin   
     ...
     -- READ DATA Channel: Read Data
        ...
        wait until (axi_rvalid='1' and axi_aclk='1' and axi_aclk'event); -- check for rvalid on the clock tick
...
BEGIN
   ...
   uut: mysmult_static_v1_0 PORT MAP ( switch, pr_int, s00_axi_aclk, s00_axi_aresetn, s00_axi_awid, s00_axi_awaddr, s00_axi_awlen, s00_axi_awsize, s00_axi_awburst, s00_axi_awlock, s00_axi_awcache,
   ...
   begin		
      -- hold reset state for 100 ns.
        ...
        --clearing irq by reading register 110100       
        READ_DATA ("110100", "00000000", "00", -- Expected Results: 0xC6D194F7
                    s00_axi_araddr, s00_axi_arlen, s00_axi_arsize, s00_axi_arburst, s00_axi_arvalid, s00_axi_arready,
                    s00_axi_rdata, s00_axi_rready, s00_axi_rvalid, s00_axi_rlast, s00_axi_aclk); 
         
         wait for 10*T;         
         switch <= '0';         --need to reset switch because it's actually a button input it's looking for
        ...
        -- ***************
           -- Send Write Address on WRITE ADDRESS Channel:
              s00_axi_awaddr <= "101100"; -- last 2 bits are 00 because they are used to address 8-bit data.
              s00_axi_awsize <= "010"; -- 32-bit word
              s00_axi_awlen <= "00000000"; -- 1 transfers
              s00_axi_awburst <= "01"; -- INCR. This way, every new data will be stored in a different mem. position
                                       --  If we have a FIFO or a fully pipelined circuit, we do not need addresses
              s00_axi_awvalid <= '1'; -- As soon as this is detected and we are aready to capture data, 'S_AXI_AWREADY' is asserted for one cycle
                       -- As soon as this happens, the value of 'awaddr' is stored (along with other control values)
                                        
           -- When S_AXI_AWREADY becomes 1 for one cycle and then 0, this is when AWVALID becomes 0
              wait until s00_axi_awready = '1'; wait until s00_axi_awready='0';
              s00_axi_awaddr <= (others => '0'); s00_axi_awvalid <= '0';
                                     
           -- Send Data via WRITE DATA Channel:                                       
              wait until (s00_axi_aclk = '0' and s00_axi_aclk'event); -- wait until falling edge
              s00_axi_wstrb <= "1111";
              -- We insert an extra cycle here, so that these signals are asserted until wready=1 (one cycle after wvalid is)                 
              s00_axi_wdata <= x"AA995577";  s00_axi_wvalid <= '1'; s00_axi_wlast <= '1'; wait for T; 
                          
              s00_axi_bready <= '1'; -- Having BREADY='1'
                                      
              wait for T; --The way the hw is designed, 'ready' goes to '1' only on the rising edge. 
                          -- We need to make sure that we capture s00_axi_bvalid
              s00_axi_wdata <= (others => '0'); s00_axi_wlast <= '0'; s00_axi_wvalid <= '0';
                                     
              -- Acknowlege;pop data receipt via WRITE RESPONSE Channel
              wait until (s00_axi_bvalid='1' and s00_axi_aclk='1' and s00_axi_aclk'event);      
              s00_axi_bready <= '0'; 
